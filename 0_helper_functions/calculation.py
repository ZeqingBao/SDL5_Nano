...
def organic_converter(target_ratio, stock_solution_conc):
    batch_size = properties["Setup"]["Organic batch size"]
    phase_volume = properties["Setup"]["Organic phase vol"]
    mix_vol = properties["Setup"]["Organic mix vol"]
...
def aqueous_converter(target_ratio, stock_solution_conc):
    batch_size = properties["Setup"]["Aqueous batch size"]
    phase_volume = properties["Setup"]["Aqueous phase vol"]
    mix_vol = properties["Setup"]["Aqueous mix vol"]
...
def converter(dataset):
    ...
    for col in organic_cols_to_convert:
        transfer_df[col] = organic_converter(
            dataset[col], properties[col]["Stock solution conc"]
        )


    for col in aqueous_cols_to_convert:
        transfer_df[col] = aqueous_converter(
            dataset[col], properties[col]["Stock solution conc"]
        )


    organic_mix_vol = properties["Setup"]["Organic mix vol"]
